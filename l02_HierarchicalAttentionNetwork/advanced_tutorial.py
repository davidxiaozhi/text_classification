...
constituent parser. Suppose our word_model involves roughly the following
steps:
...
* From there, use a neural network and the embeddings
  ...
  scratched the wall", at some point in the word_model, we will want to combine
...
word_model like the CRF is really essential for strong performance on NER.
Familiarity with CRF's is assumed. Although this name sounds scary, all
the word_model is is a CRF but where an LSTM provides the features. This is
an advanced word_model though, far more complicated than any earlier word_model in
...
transition. The emission potential for the word at train_batch :math:`i` comes
